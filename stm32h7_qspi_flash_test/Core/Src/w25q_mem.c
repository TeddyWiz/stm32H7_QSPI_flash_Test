...
W25Q_STATE W25Q_Init(void) {
	...
	if (!w25q_status.ADP) {
		...
		if (state != W25Q_OK)
			return state;
		#if Debug_25Q
		printf("[%s > %s : %d]sttus ADP read1= %02x \r\n",__FILE__, __FUNCTION__, __LINE__, buf_reg );
		#endif
		buf_reg |= 0b10; 	// set ADP bit
		#if Debug_25Q
		printf("[%s > %s : %d]sttus ADP write= %02x \r\n",__FILE__, __FUNCTION__, __LINE__, buf_reg );
		#endif
		...
		if (state != W25Q_OK)
			return state;
		#if Debug_25Q
		printf("[%s > %s : %d]sttus ADP read2= %02x \r\n",__FILE__, __FUNCTION__, __LINE__, buf_reg );
		#endif
	}
...
W25Q_STATE W25Q_ReadStatusReg(u8_t *reg_data, u8_t reg_num) {
	...
	com.AddressSize = HAL_OSPI_ADDRESS_NONE;
	com.Address = 0x0U;
	...
	//com.DdrHoldHalfCycle = HAL_OSPI_DDR_HHC_ANALOG_DELAY;
	...
	if (HAL_OSPI_Receive(&hospi1, reg_data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_WriteStatusReg(u8_t reg_data, u8_t reg_num) {
	...
	if (HAL_OSPI_Transmit(&hospi1, &reg_data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_ReadStatusStruct(W25Q_STATUS_REG *status) {
	...
	if (state != W25Q_OK)
		return state;
	#if Debug_25Q
	printf("[%s > %s : %d]W25Q_ReadStatusReg0 = %02x \r\n",__FILE__, __FUNCTION__, __LINE__, SRs[0] );
	#endif
	...
	#if Debug_25Q
	printf("[%s > %s : %d]W25Q_ReadStatusReg1 = %02x \r\n",__FILE__, __FUNCTION__, __LINE__, SRs[1] );
	#endif
	// third portion
	...
	if (state != W25Q_OK)
		return state;
	#if Debug_25Q
	printf("[%s > %s : %d]W25Q_ReadStatusReg2 = %02x \r\n",__FILE__, __FUNCTION__, __LINE__, SRs[2] );
	#endif
...
W25Q_STATE W25Q_ReadRaw(u8_t *buf, u16_t data_len, u32_t rawAddr) {
	...
	com.AddressSize = HAL_OSPI_ADDRESS_32_BITS;
#else
...
W25Q_STATE W25Q_ReadID(u8_t *buf) {
	...
	if (HAL_OSPI_Receive(&hospi1, buf, HAL_OSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
	#if Debug_25Q
